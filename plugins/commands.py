...
#                     InlineKeyboardButton("𝑴𝒐𝒗𝒊𝒆 𝑰𝒅𝒆𝒂", url="https://t.me/Hell_King_69_Bot/Hell_King_Movie_Details")
#                 ],[
#                      InlineKeyboardButton("🎮 Mini Games", callback_data="games") # Add this line
#                    ]]
#         reply_markup = InlineKeyboardMarkup(buttons)
...
# async def settings(client, message, query: CallbackQuery):
#     userid = message.from_user.id if message.from_user else None
#     if not userid:
...
# # এই কোডটি commands.py ফাইলের শেষে যোগ করুন

# @Client.on_message(filters.command("watch_online_mode") & filters.user(ADMINS))
# async def set_watch_online_mode(client, message):
#     """Command to set the 'Watch Online' button mode."""
#     try:
#         option = message.text.split(" ", 1)[1].strip().lower()
#         if option not in ['on', 'off', 'true', 'false']:
#             raise ValueError
...
#         is_premium_only_mode = option in ['on', 'true']
#     except (IndexError, ValueError):
#         await message.reply_text("<b>💔 Invalid option. Please use '/watch_online_mode on' or '/watch_online_mode off'.</b>")
#         return

#     try:
#         # Note: You need to implement 'update_watch_online_mode' in your config_db.py
#         await mdb.update_watch_online_mode(is_premium_only_mode)
        
#         response_text = (
#             "<b>✅ 'Watch Online' button mode set to ON.\nOnly premium users can now use it.</b>" if is_premium_only_mode
#             else "<b>✅ 'Watch Online' button mode set to OFF.\nAll users can now use it.</b>"
#         )
#         await message.reply_text(response_text)
#     except Exception as e:
...
from database.ia_filterdb import Media, Media2, get_file_details, unpack_new_file_id, get_bad_files
from database.users_chats_db import db, delete_all_msg
from info import CHANNELS, FSUB_PICS, ADMINS, LOG_CHANNEL, PICS, BATCH_FILE_CAPTION, CUSTOM_FILE_CAPTION, PROTECT_CONTENT, CHNL_LNK, REQST_CHANNEL, GRP_LNK, SUPPORT_CHAT_ID, MAX_B_TN, VERIFY, REACTIONS, HOW_TO_VERIFY, PICS, DEENDAYAL_VERIFIED_LOG, SUBSCRIPTION, DEENDAYAL_VERIFY_EXPIRE, DEENDAYAL_MOVIE_UPDATE_CHANNEL_LNK, STREAM_MODE, EMOJI_MODE, OWNER_LNK, OWNER_UPI_ID, QR_CODE, DELETE_TIME,DAILY_LIMIT
from utils import get_settings, get_size, is_subscribed,  save_group_settings, temp, verify_user, check_token, check_verification, get_token, get_shortlink, get_tutorial, get_time
from database.connections_mdb import active_connection
...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
                    ...
                    InlineKeyboardButton("𝑴𝒐𝒗𝒊𝒆 𝑰𝒅𝒆𝒂", url="https://t.me/Hell_King_69_Bot/Hell_King_Movie_Details")
                     ...
                     InlineKeyboardButton("🎮 Mini Games", callback_data="games")
                   ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        current_time = datetime.now(pytz.timezone(TIMEZONE))
        curr_time = current_time.hour        
    ...
    if data.startswith("sendfiles"):
        ...
        for file in files:
            ...
            if not is_premium:
                ...
                if daily_downloads >= DAILY_LIMIT:
                    await client.send_message(user_id, f"<b>Hey, you completed the daily quota {DAILY_LIMIT}/{DAILY_LIMIT}.</b>\n\nSome files could not be sent. Upgrade to premium for unlimited access. /plan")
                    break
                
                await db.increment_daily_downloads(user_id)
                new_daily_downloads = await db.get_daily_downloads(user_id)
                limit_text = f"{new_daily_downloads}/{DAILY_LIMIT}"
            else:
                limit_text = "♾️"
            ...
            # f_caption = f"""<b><a href="https://t.me/Hell_king_69_Bot"><blockquote>{title}</blockquote></a></b>\n\n Powered By ⚜️ : The Hell King\n Size ⚙️: {size}\n\nDᴀɪʟʏ Fɪʟᴇ Lɪᴍɪᴛ: {limit_text}\n\n⚠️ ꜰɪʟᴇ ᴡɪʟʟ ʙᴇ ᴅᴇʟᴇᴛᴇ ᴀꜰᴛᴇʀ 5 \n\n<b><i>ᴘʟᴇᴀꜱᴇ ꜰᴏʀᴡᴀʀᴅ ᴛʜɪꜱ ꜰɪʟᴇ ᴛᴏ ꜱᴏᴍᴇᴡʜᴇʀᴇ ᴇʟꜱᴇ ᴀɴᴅ ꜱᴛᴀʀᴛ ᴅᴏᴡɴʟᴏᴀᴅɪɴɢ ᴛʜᴇʀᴇ</i></b>"""
            f_caption = f"""<b><a href="https://t.me/Hell_king_69_Bot"><blockquote>{title}</blockquote></a></b>\n\n<b>Powered By ⚜️ :<a href="https://t.me/Hell_king_69_Bot"> The Hell King</a>\n Size ⚙️: {size}</b>\n\n<blockquote> Dᴀɪʟʏ Fɪʟᴇ Lɪᴍɪᴛ: {limit_text}</blockquote>\n\n<b>⚠️ ꜰɪʟᴇ ᴡɪʟʟ ʙᴇ ᴅᴇʟᴇᴛᴇ ᴀꜰᴛᴇʀ <code>{get_time(DELETE_TIME)}</code> </b>\n\n<b><i>ᴘʟᴇᴀꜱᴇ ꜰᴏʀᴡᴀʀᴅ ᴛʜɪꜱ ꜰɪʟᴇ ᴛᴏ ꜱᴏᴍᴇᴡʜᴇʀᴇ ᴇʟꜱᴇ ᴀɴᴅ ꜱᴛᴀʀᴛ ᴅᴏᴡɴʟᴏᴀᴅɪɴɢ ᴛʜᴇʀᴇ</i></b>"""

            if not is_premium and not await check_verification(client, message.from_user.id) and VERIFY == True:
                btn = [[
                    InlineKeyboardButton("✅ Cʟɪᴄᴋ ʜᴇʀᴇ ᴛᴏ ᴠᴇʀɪғʏ ✅", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start=", file_id))
                    ],[
                    InlineKeyboardButton("⚡ Hᴏᴡ ᴛᴏ ᴠᴇʀɪғʏ ⚡", url=HOW_TO_VERIFY)
                ...
                await asyncio.sleep(180)
                await l.delete()
                # Since verification is required, we stop processing this batch
                return
        ...
        if filesarr:
            ...
            await client.send_message(
        chat_id=message.from_user.id,
        text="<b>ʏᴏᴜʀ ᴠɪᴅᴇᴏ / ꜰɪʟᴇ ɪꜱ ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟʏ ᴅᴇʟᴇᴛᴇᴅ !!</b>"
    )
        return
        ...
        if not is_premium:
            daily_downloads = await db.get_daily_downloads(user_id)
            if daily_downloads >= DAILY_LIMIT:
                return await message.reply_text(f"<b>Hey, you completed the daily quota {DAILY_LIMIT}/{DAILY_LIMIT}.</b>\n\nUpgrade to premium for unlimited access. /plan")

        if temp.SHORT.get(user_id) is None:
            return await message.reply_text(text="<b>Please Search Again in Group</b>")
        
        chat_id = temp.SHORT.get(user_id)
        settings = await get_settings(chat_id)
        if not is_premium and settings['is_shortlink']:
            files_ = await get_file_details(file_id)
    ...
    # This is the main logic for single file sending
    user_id = message.from_user.id
    is_premium = await db.has_premium_access(user_id)
    limit_text = ""
    ...
    if not is_premium:
        ...
        # Increment first, then get the new value for the caption
        await db.increment_daily_downloads(user_id)
        new_daily_downloads = await db.get_daily_downloads(user_id)
        limit_text = f"{new_daily_downloads}/{DAILY_LIMIT}"
    else:
    ...
    if not files_:
        pre, file_id = ((base64.urlsafe_b64decode(data + "=" * (-len(data) % 4))).decode("ascii")).split("_", 1)
        # Re-fetch file details with the decoded ID
        files_ = await get_file_details(file_id)
        if not files_:
            return await message.reply('ɴᴏ ꜱᴜᴄʜ ꜰɪʟᴇ ᴇxɪꜱᴛꜱ !')
    ...
    if not is_premium and not await check_verification(client, message.from_user.id) and VERIFY == True:
        ...
        # We must revert the increment if verification is required and fails
        await db.users.update_one({"id": user_id}, {"$inc": {"daily_downloads": -1}})
        l = await message.reply_text(
            text=f"<b>♨️ YOU ARE NOT VERIFIED !\nKINDLY VERIFY TO CONTINUE & YOU CAN GET UNLIMITED ACCESS FOR {DEENDAYAL_VERIFY_EXPIRE} HOURS ✅\n\n<blockquote>⚜️ इस BOT से MOVIE प्राप्त करने के लिए आपको VERIFY करना आवश्यक है VERIFY करने के बाद आप {DEENDAYAL_VERIFY_EXPIRE}  घंटे  तक UNLIMITED MOVIES प्राप्त कर सकते हो।</blockquote> \n\n<blockquote>🔥 अगर आप VERIFY नहीं करना चाहते तो आप PREMIUM ले सकते हो, PREMIUM लेने के बाद आप UNLIMITED MOVIES प्राप्त कर पाओगे और आपको VERIFY करने कोई जरूरत नहीं है\n\nPLAN DETAILS के लिए CLICK करें /plan</blockquote></b>",
            protect_content=False,
            reply_markup=InlineKeyboardMarkup(btn)
